...
ENV DEBIAN_FRONTEND=noninteractive STEAMCMDDIR=/serverdata/steamcmd SERVERDIR=/serverdata/serverfiles
...
RUN useradd -m -u 99 -U -s /bin/bash steam && mkdir -p /serverdata/steamcmd /serverdata/serverfiles && chown -R steam:steam /serverdata
USER steam
WORKDIR /serverdata/steamcmd
