// Persistent dedicated Invasion support for the RWR Unraid container.
//
// Vanilla dedicated Invasion intentionally leaves save() and load() empty.
// This subclass uses the same metagame serialization flow as RWR's campaign
// mode, allowing the existing AutoSaver tracker and uninit() hook to preserve
// mission progress in the persistent savegames directory.

class RwrUnraidPersistentInvasion : GameModeInvasion {
	RwrUnraidPersistentInvasion(UserSettings@ settings) {
		super(settings);
		// Dedicated start_invasion.as defaults this to false. Always request a
		// restore; load() safely initializes a fresh rotation when no save exists.
		settings.m_continue = true;
	}

	void save() {
		_log("saving persistent invasion metagame", 1);
	...
	void load() {
		_log("loading persistent invasion metagame", 1);
