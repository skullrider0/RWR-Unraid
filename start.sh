...
SERVER_BIN=""
for candidate in "$SERVERDIR/rwr_server" "$SERVERDIR/rwr_gameserver/rwr_server" "$SERVERDIR/rwr_gameserver/launch_server"; do
  if [ -x "$candidate" ]; then SERVER_BIN="$candidate"; break; fi
done
if [ -z "$SERVER_BIN" ]; then SERVER_BIN="$(find "$SERVERDIR" -maxdepth 4 -type f \( -name 'rwr_server' -o -name 'launch_server' \) -perm -111 2>/dev/null | head -n 1 || true)"; fi
